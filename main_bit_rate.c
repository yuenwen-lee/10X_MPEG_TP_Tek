...
static void tp_dump_bit_rate_between_pcr(uint32_t *tp_buff, uint32_t tp_buff_size,
					 int32_t pid_PCR, uint32_t flag_NULL);
...
int main(int argc, char *argv[])
{
    ...
    printf("\n\n");
    tp_dump_bit_rate_between_pcr(tp_buff, tp_buff_size, pid_pcr, flag_exclude_NULL);
...
static void tp_dump_bit_rate_between_pcr(uint32_t *tp_buff,
					 uint32_t tp_buff_size,
					 int32_t target_pcr_pid,
					 uint32_t flag_exclude_NULL)
{
    ...
    for (tp_indx = 0; tp_indx < tp_numb; ++tp_indx) {
        ...
        if (rt) {
            ...
            printf("-%1d-  %8u: %02d:%02d:%02d.%03d.%03d, (%7u, %3u), %6u msec",
